...
**前置知识建议：** 建议先学完 [1-1 大模型认知与工程概览](1-1-大模型认知与工程概览.md)、[1-3 RAG、微调、续训与智能体](1-3-RAG、微调、续训与智能体.md) 中的智能体部分，了解「何时需要智能体、工具调用」；若已用 [2-RAG-搭建企业私有&个人知识库](2-RAG-搭建企业私有&个人知识库.md) 搭过知识库，本篇里「知识库 + 智能体」会更好理解。

**入门阅读提示：** 本章偏动手，建议先通读第 1 节建立「智能体 = 大脑 + 记忆 + 工具 + 规划 + 行动」和「三个 level」的图景，再按顺序做：level 1 用 Cherry Studio 或豆包练手 → level 3 用 Coze 跟做 2 ～ 3 个案例（情感主持、高考、记账）→ 若需工作流再学第 6 节及案例链接。Coze 与 Dify 二选一先精通一个即可，两者思路类似（提示词 + 插件/工具 + 知识库/变量）。
...
案例 1：详细过程见 [Coze 案例：一键生成行业调研 PPT](案例与源码-3-Coze&Dify平台工作流智能体/3.1-Coze案例：一键生成行业调研PPT/3.1-一键生成行业调研PPT.md)

案例 2：详细过程见 [Coze 案例：复刻爆款视频](案例与源码-3-Coze&Dify平台工作流智能体/3.2-Coze案例：复刻爆款视频/3.2-复刻爆款视频.md)
...
1、产品营销海报生成：[Coze](案例与源码-3-Coze&Dify平台工作流智能体/3.3-Coze案例：产品营销海报生成/3.3-产品营销海报生成.md)

2、客户投诉分类助手：[Dify](案例与源码-3-Coze&Dify平台工作流智能体/3.5-Dify案例：客户投诉分类助手/3.5-客户投诉分类助手.md)

3、一键生成行业调研报告：[Dify](案例与源码-3-Coze&Dify平台工作流智能体/3.4-Dify案例：一键生成行业调研报告/3.4-一键生成行业调研报告.md)

4、客服对话记录分析：[Coze](案例与源码-3-Coze&Dify平台工作流智能体/3.6-Coze案例：客服对话记录分析/3.6-客服对话记录分析-Coze.md) / [Dify](案例与源码-3-Coze&Dify平台工作流智能体/3.7-Dify案例：客服对话记录分析/3.7-客服对话记录分析-Dify.md)

5、商品评论分析：[Coze](案例与源码-3-Coze&Dify平台工作流智能体/3.8-Coze案例：商品评论分析/3.8-商品评论分析-商品评论分析-Coze.md) / [Dify](案例与源码-3-Coze&Dify平台工作流智能体/3.9-Dify案例：商品评论分析/3.9-商品评论分析-Dify.md)

6、商品营销卖点提炼：[Coze](案例与源码-3-Coze&Dify平台工作流智能体/3.10-Coze案例：商品营销卖点提炼/3.10-商品营销卖点提炼-Coze.md)
...
- **智能体**：以 LLM 为「大脑」，具备记忆、工具调用、规划与行动；从易到难分为 level 1（提示词立人设）、level 2（工作流）、level 3（自主任务拆解）。
- **Level 1**：Cherry Studio、豆包、讯飞等用提示词即可创建简易智能体，适合练手和快速验证人设。
- **Coze（扣子）**：零代码、模板多、与字节系产品集成；核心是「提示词（角色/技能/限制）+ 插件（联网、搜索等）+ 知识库 + 变量/数据库」；案例覆盖情感主持（纯提示词）、高考报考（插件+知识库）、家庭记账（变量+数据库）。
- **Dify**：偏开发者与团队，支持 Agent、RAG、Workflow；搭建 Agent 时需配置提示词、工具（如 DuckDuckGo）、对话开场白与内容审查；案例有时事评论助手、北京旅行助手（工具调用与变量）。
- **工作流**：固定步骤的自动化链路，Coze/Dify 均支持；复杂需求（一键生成 PPT、复刻爆款视频等）见第 6 节及「商户运营管家」项目链接。
- **后续**：用代码调用 Dify/Coze 工作流见 [第 4 章](4-Python调用Dify平台工作流.md)、[第 5 章](5-Python调用Coze平台工作流.md)；DeepResearch 等为拓展阅读，了解即可。

**建议下一步：** 在 Coze 或 Dify 上选一个案例（如深夜情感主持或北京旅行助手）从头跟做一遍并发布；再选一个带插件或知识库的案例体会「工具 + 提示词」的配合。若要做自动化流水线，可进入 [工作流案例](案例与源码-3-Coze&Dify平台工作流智能体/3.1-Coze案例：一键生成行业调研PPT/3.1-一键生成行业调研PPT.md) 学习。
